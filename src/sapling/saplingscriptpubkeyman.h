...
#ifndef HU_SAPLING_SAPLINGSCRIPTPUBKEYMAN_H
#define HU_SAPLING_SAPLINGSCRIPTPUBKEYMAN_H
...
/**
 * KHU Lock Metadata - tags Sapling notes that are ZKHU locks
 * This allows the standard Sapling witness pipeline to track KHU lock notes
 * alongside regular Shield notes.
 */
struct KHULockMeta
{
    bool is_khu_lock{false};       // True if this note is a KHU_LOCK
    uint32_t lock_height{0};       // Block height when lock was created
    bool is_mature{false};          // True if maturity period (4320 blocks) passed

    SERIALIZE_METHODS(KHULockMeta, obj)
    {
        READWRITE(obj.is_khu_lock);
        READWRITE(obj.lock_height);
        READWRITE(obj.is_mature);
    }

    friend bool operator==(const KHULockMeta& a, const KHULockMeta& b) {
        return (a.is_khu_lock == b.is_khu_lock &&
...
public:
    ...
    /**
     * KHU Lock Metadata - identifies this note as a ZKHU lock note
     * Set when a KHU_LOCK transaction is detected in FindMySaplingNotes
     */
    KHULockMeta khu_lock_meta;
    ...
    friend bool operator==(const SaplingNoteData& a, const SaplingNoteData& b) {
        return (a.ivk == b.ivk &&
                ...
                a.address == b.address &&
                a.memo == b.memo &&
                a.khu_lock_meta == b.khu_lock_meta);
    }
...
#endif // HU_SAPLING_SAPLINGSCRIPTPUBKEYMAN_H
