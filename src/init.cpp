...
/** Sanity checks
 *  Ensure that PIV2-Core is running in a usable environment with all
...
#endif
    LogPrintf("PIV2 Chain version %s\n", version_string);
}
...
bool AppInitParameterInteraction()
{
    ...
    // Legacy flag silently ignored for backwards compatibility
...
{
    // Make sure only a single PIV2 process is using the data directory.
...
bool AppInitMain()
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile because if PIV2 is started in the future " /* Continued */
                  "from a different location. It will be unable to locate the current data files. There could " /* Continued */
                  "also be data loss if PIV2 is started while in a temporary directory.\n",
...
// ********************************************************* Step 5: Verify wallet database integrity
    ...
    // Log PIV2 DMM + Finality consensus parameters
    LogPrintf("PIV2 consensus params: quorum_size=%d, threshold=%d, rotation=%d, leader_timeout=%ds, max_reorg=%d\n",
    ...
    while (!fLoaded && !ShutdownRequested()) {
        ...
        do {
            ...
            try {
                ...
                // KHU: Initialize ZKHU database (Phase 4/5 - Sapling ZKHU lock)
                ...
                // Load previous session sporks if we have them.
