...
namespace Standard {
    ...
    CWDestination DecodeDestination(const std::string& strAddress)
    {
        ...
        return DecodeDestination(strAddress, isExchange);
    }

    CWDestination DecodeDestination(const std::string& strAddress, bool& isExchange)
    {
        bool isShielded = false;
        return DecodeDestination(strAddress, isExchange, isShielded);
    }

    // Simplified decoding - only isExchange and isShielded flags
    CWDestination DecodeDestination(const std::string& strAddress, bool& isExchange, bool& isShielded)
    {
        CWDestination dest;
        CTxDestination regDest = ::DecodeDestination(strAddress, isExchange);
...
std::string Destination::ToString() const
{
    ...
    CChainParams::Base58Type addrType = isExchange ? CChainParams::EXCHANGE_ADDRESS
