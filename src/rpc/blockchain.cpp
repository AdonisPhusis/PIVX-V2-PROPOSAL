...
UniValue blockheaderToJSON(const CBlockIndex* tip, const CBlockIndex* blockindex)
{
    ...
    // PIV2 finality: block is finalized after nHuMaxReorgDepth confirmations
...
// Returns the best finalized block (depth >= PIV2_FINALITY_DEPTH)
UniValue getbestfinalized(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            "getbestfinalized\n"
            "\nReturns the block hash of the best finalized block (depth >= 12).\n"
            "\nResult:\n"
            "{\n"
            "  \"blockhash\" : \"hash\",      (string) The block hash hex encoded\n"
            "  \"height\" : n,              (numeric) The block height or index\n"
            "  \"finality_depth\" : n       (numeric) The PIV2 finality depth threshold\n"
            "}\n"
            "\nExamples:\n" +
    ...
    int finalizedHeight = std::max(0, chainHeight - nHuFinalityDepth);

    if (finalizedHeight <= 0 || chainHeight < nHuFinalityDepth) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "No finalized blocks yet (chain too short)");
    }

    CBlockIndex* pindex = chainActive[finalizedHeight];
    if (!pindex) {
        throw JSONRPCError(RPC_INTERNAL_ERROR, "Unable to find finalized block");
    }

    result.pushKV("blockhash", pindex->GetBlockHash().GetHex());
    result.pushKV("height", pindex->nHeight);
    result.pushKV("finality_depth", nHuFinalityDepth);
    return result;
}
