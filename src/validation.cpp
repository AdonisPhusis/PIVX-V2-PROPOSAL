                              ...
                              std::vector<COutPoint>& coins_to_uncache) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
...

    {
        ...
        if (tx.nType == CTransaction::TxType::KHU_DOMC_COMMIT ||
            ...
            if (!khudb->ReadKHUState(chainHeight, huState)) {
            ...
            if (tx.nType == CTransaction::TxType::KHU_DOMC_COMMIT) {
                if (!ValidateDomcCommitTx(tx, state, huState, nextBlockHeight, consensus)) {
            ...
            // Validate DOMC reveal transaction
                ...
                if (!ValidateDomcRevealTx(tx, state, huState, nextBlockHeight, consensus)) {
...
{
    // PIVHU chain has V6 active from genesis - economy governed entirely by KHU R% yield
...
bool CheckTxInputs(const CTransaction& tx, CValidationState& state, const CCoinsViewCache& inputs, int nSpendHeight)
{
    ...
    // All transactions must have value_in >= value_out
...
DisconnectResult DisconnectBlock(CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false)
{
    ...
    // KHU: Disconnect KHU state (Phase 4: Undo LOCK/UNLOCK operations)
...
static bool ConnectBlock(const CBlock& block, CValidationState& state, CBlockIndex* pindex, CCoinsViewCache& view, bool fJustCheck = false) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    if (pindex->pprev && pindex->phashBlock && hu::huFinalityHandler &&
        hu::huFinalityHandler->HasConflictingFinality(pindex->nHeight, pindex->GetBlockHash())) {
    ...
    // HU V6.0: FINALITY VIA MASTERNODES (HU Finality mandatory after 12 blocks)
    // After V6 activation, blocks older than nHuMaxReorgDepth confirmations MUST have HU finality
    // This transfers finality from lockrs to masternodes via HU quorum (12 MN, 8 sigs)
    ...
    if (isV6UpgradeEnforced && pindex->pprev && hu::huFinalityHandler) {
        // Check if the block we're building on (nHuFinalityDepth blocks back) has HU finality
        CBlockIndex* pindexCheck = pindex->pprev;
        int depth = 0;

        // Walk back to find block at nHuFinalityDepth
        while (pindexCheck && depth < nHuFinalityDepth) {
            pindexCheck = pindexCheck->pprev;
            depth++;
        }

        // If we have nHuFinalityDepth+ blocks of history after V6, enforce HU finality on ancestors
        if (pindexCheck && depth == nHuFinalityDepth &&
            consensus.NetworkUpgradeActive(pindexCheck->nHeight, Consensus::UPGRADE_V6_0)) {

            // Check if the block at nHuFinalityDepth has HU finality
            // Only enforce if HU enforcement spork is active OR if we're past V6 + depth
            if (sporkManager.IsSporkActive(SPORK_23_HU_FINALITY_ENFORCEMENT)) {
                if (!hu::huFinalityHandler->HasFinality(pindexCheck->nHeight, pindexCheck->GetBlockHash())) {
                    // Don't reject during initial sync - only warn
                    if (g_tiertwo_sync_state.IsSynced()) {
                        return state.DoS(10, error("%s: V6 finality requires HU finality for block at depth %d (height %d)",
                                                   __func__, nHuFinalityDepth, pindexCheck->nHeight),
                                         REJECT_INVALID, "v6-hu-finality-required");
                    } else {
                        LogPrint(BCLog::HU, "HU: Warning - block at depth %d lacks HU finality (syncing)\n", nHuFinalityDepth);
                    }
                }
            }
        }
    ...
    if (isV6UpgradeEnforced && !IsCoinbaseValueValid(block.vtx[0], state)) {
        // pass the state returned by the function above
        return false;
    }
    ...
    if (consensus.NetworkUpgradeActive(pindex->nHeight, Consensus::UPGRADE_V6_0)) {
        LogPrint(BCLog::HU, "ConnectBlock: Calling ProcessHUBlock height=%d fJustCheck=%d\n",
                 pindex->nHeight, fJustCheck);
        if (!ProcessHUBlock(block, pindex, view, state, consensus, fJustCheck)) {
            LogPrint(BCLog::HU, "ConnectBlock: ProcessHUBlock FAILED at height=%d\n", pindex->nHeight);
            return error("%s: ProcessHUBlock failed for %s", __func__, block.GetHash().ToString());
        }
        LogPrint(BCLog::HU, "ConnectBlock: ProcessHUBlock SUCCESS at height=%d\n", pindex->nHeight);
    }
    ...
    // NOTE: ProcessHUBlock is now called earlier, before fJustCheck return,
...
bool AcceptBlockHeader(const CBlock& block, CValidationState& state, CBlockIndex** ppindex, CBlockIndex* pindexPrev)
{
    ...
    if (block.GetHash() != Params().GetConsensus().hashGenesisBlock && hu::huFinalityHandler) {
        if (hu::huFinalityHandler->HasConflictingFinality(pindexPrev->nHeight + 1, hash)) {
            return state.DoS(10, error("%s: conflicting with HU finality", __func__), REJECT_INVALID, "bad-hu-finality");
        }
    }
    if (pindex == nullptr)
...
bool TestBlockValidity(CValidationState& state, const CBlock& block, CBlockIndex* const pindexPrev, bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckBlockSig)
{
    ...
    // HU Finality: Check for conflicting finalized blocks
    if (hu::huFinalityHandler && hu::huFinalityHandler->HasConflictingFinality(pindexPrev->nHeight + 1, block.GetHash())) {
        return state.DoS(10, error("%s: conflicting with HU finality", __func__), REJECT_INVALID, "bad-hu-finality");
    }
