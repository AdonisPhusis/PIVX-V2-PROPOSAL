        ...
        for (uint32_t i = 0; i < tx->sapData->vShieldedOutput.size(); i++) {
            ...
            if (txIsOurs) {
                ...
                if (ndIt != wtx->mapSaplingNoteData.end()) {
                    ...
                    // Update lock_height for KHU_LOCK notes when first witnessed
                    // This happens when the LOCK tx is confirmed in a block
                    if (nd->khu_lock_meta.is_khu_lock) {
                        // Set lock_height if not already set (first confirmation)
                        if (nd->khu_lock_meta.lock_height == 0) {
                            nd->khu_lock_meta.lock_height = chainHeight;
                            LogPrint(BCLog::HU, "IncrementNoteWitnesses: KHU_LOCK note confirmed, "
                                     "txid=%s, lock_height=%d, witnesses.size=%zu\n",
                                     hash.ToString().substr(0, 16), chainHeight, nd->witnesses.size());
                        } else {
                            LogPrint(BCLog::HU, "IncrementNoteWitnesses: processing KHU_LOCK note, "
                                     "txid=%s, height=%d, lock_height=%d, witnesses.size=%zu\n",
                                     hash.ToString().substr(0, 16), chainHeight,
                                     nd->khu_lock_meta.lock_height, nd->witnesses.size());
                        }
                    }
                }
            }
...
std::pair<mapSaplingNoteData_t, SaplingIncomingViewingKeyMap> SaplingScriptPubKeyMan::FindMySaplingNotes(const CTransaction &tx) const
{
    ...
    for (uint32_t i = 0; i < tx.sapData->vShieldedOutput.size(); ++i) {
        ...
        for (auto it = wallet->mapSaplingFullViewingKeys.begin(); it != wallet->mapSaplingFullViewingKeys.end(); ++it) {
            ...
            // Tag KHU_LOCK notes for the standard Sapling witness pipeline
            // This allows IncrementNoteWitnesses to maintain witnesses for ZKHU lock notes
            if (tx.nType == CTransaction::TxType::KHU_LOCK) {
                nd.khu_lock_meta.is_khu_lock = true;
                nd.khu_lock_meta.lock_height = 0;  // Will be set when tx is confirmed
                nd.khu_lock_meta.is_mature = false;
                LogPrint(BCLog::HU, "FindMySaplingNotes: detected KHU_LOCK note, txid=%s, op.n=%d\n",
