...
UniValue sendtoaddress(const JSONRPCRequest& request)
{
    ...
    const CWDestination& destination = Standard::DecodeDestination(addrStr, isExchange, isShielded);
    if (!Standard::IsValidDestination(destination))
    ...
    if (isShielded) {
        ...
        return ShieldSendManyTo(pwallet, sendTo, commentStr, toStr, 1, subtractFeeFromAmount);
    }
...
UniValue getstakingstatus(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "\nFor ZKHU yield information, use: getkhustate, khulistlocked\n"
            "\nFor chain status, use: getblockchaininfo\n"

            "\nResult:\n"
            "{\n"
            "  \"deprecated\": true,\n"
            "  \"message\": \"MN-only consensus - use KHU staking commands instead\",\n"
            "  \"use_instead\": [\"getkhustate\", \"khulistlocked\", \"getblockchaininfo\"]\n"
            "}\n"
    ...
    obj.pushKV("message", "MN-only consensus - use KHU staking commands instead");

    UniValue alternatives(UniValue::VARR);
    alternatives.push_back("getkhustate");
    alternatives.push_back("khulistlocked");
    alternatives.push_back("getblockchaininfo");
    obj.pushKV("use_instead", alternatives);
...
void RegisterWalletRPCCommands(CRPCTable &tableRPC)
{
    ...
    RegisterHUWalletRPCCommands(tableRPC);
}
