    ...
    switch(tx.nType) {
        case CTransaction::TxType::PROREG: {
            ...
            if (mapProTxAddresses.count(pl.addr) || mapProTxPubKeyIDs.count(pl.keyIDOwner) ||
                    mapProTxOperatorKeyIDs.count(pl.pubKeyOperator.GetID())) {
                return true;
            }
            if (!pl.collateralOutpoint.hash.IsNull()) {
        ...
        case CTransaction::TxType::PROUPREG: {
            ...
            auto it = mapProTxOperatorKeyIDs.find(pl.pubKeyOperator.GetID());
            return it != mapProTxOperatorKeyIDs.end() && it->second != pl.proTxHash;
        }
